      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
             ...
             <div className="text-center">
                {settings?.siteName ? (
                  settings.siteName.split(' et ').length === 2 ? (
                    <>
                      <span className="block font-serif text-sm italic">{settings.siteName.split(' et ')[0]}</span>
                      <span className="block text-[8px]">&</span>
                      <span className="block font-serif text-sm italic">{settings.siteName.split(' et ')[1]}</span>
                    </>
                  ) : (
                    <span className="block font-serif text-sm">{settings.siteName}</span>
                  )
                ) : (
                  <>
        ...
        <h3 className="font-serif text-xl tracking-widest text-gray-800 mb-8 uppercase">
          {settings?.siteName ? (
            settings.siteName.split(' et ').length === 2 ? (
              <>
                {settings.siteName.split(' et ')[0].toUpperCase()} <span className="italic text-sm mx-1">et</span> {settings.siteName.split(' et ')[1].toUpperCase()}
              </>
            ) : (
            ...
            <div className="flex gap-4">
                ...
                {settings?.social?.twitter && (
                  <a href={settings.social.twitter} target="_blank" rel="noopener noreferrer" aria-label="Twitter">
                    <Twitter size={20} className="cursor-pointer hover:text-[#5d6e64] transition"/>
                  </a>
                )}
                {settings?.email && (
        ...
        <div className="text-[10px] text-gray-400">
            © 2025 {settings?.siteName || 'Vivi et Margot'}. All rights reserved.
        </div>
