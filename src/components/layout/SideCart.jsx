...
export default function SideCart() {
  ...
  // Si le panier n'est pas ouvert, on ne rend rien (pour tester) ou on le cache via CSS
  // Pour le débogage, on va utiliser une approche simple : classe 'hidden' vs 'block' ou translate.
  ...
  return (
    <div className={`fixed inset-0 z-50 ${isCartOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      {/* 1. OVERLAY (Fond noir) */}
      <div 
        className={`fixed inset-0 bg-black/50 transition-opacity duration-300 ${isCartOpen ? 'opacity-100' : 'opacity-0'}`} 
        onClick={closeCart}
      />
      ...
      >
        <div className="flex flex-col h-full">
                ...
                <div key={item.id} className="flex items-center space-x-4 border-b border-gray-100 pb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center mt-2 border border-gray-200 w-fit">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 text-gray-500 hover:bg-gray-100"><Minus size={12} /></button>
                      <span className="px-3 text-xs">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 text-gray-500 hover:bg-gray-100"><Plus size={12} /></button>
                    </div>
                  </div>
                  <button onClick={() => removeItem(item.id)} className="text-gray-400 hover:text-red-500 transition p-2"><Trash2 size={16} /></button>
                </div>
              ))
            )}
          </div>
          ...
          <div className="p-6 border-t border-gray-200 bg-white">
            ...
            <button 
              className={`w-full py-3 text-sm uppercase tracking-widest text-white transition duration-300 ${cart.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#5d6e64] hover:bg-[#4a5850]'}`}
              disabled={cart.length === 0}
            >
              Checkout
            </button>
            <button onClick={closeCart} className="w-full mt-3 text-xs text-gray-500 hover:underline text-center">
